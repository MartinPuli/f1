...
A warm function rejects overlapping decisions for the same driver in the same browser session with 409. Different drivers can request concurrently. There is no project-wide concurrency cap or shared driving quota. The local concurrency guard isn't a distributed lock. Timeouts bound upstream calls to 12 seconds. The API also accepts legacy multi-driver batches; a failed call aborts that batch’s remaining requests, though TypeSafe may already have accepted and billed them. Bodies have a five-second read deadline, with 60 KB for Vercel decisions, 3.5 MB for recordings, and 256 KB for upstream JSON.
