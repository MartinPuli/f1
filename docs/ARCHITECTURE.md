...
New frames contain a timestamp and ten arrays in stable driver order: x, z, heading, speed, steering, progress, lap, finished flag, distance, battery, tire condition, damage, numeric codes for power, lane, and pace, a retired flag, engine temperature, suspension and wing damage, spin duration, and a cooling-leak flag. Older 15- and 16-value telemetry frames also remain valid. Ten-car races capture at 5 Hz to keep longer recordings within the 3.5 MB upload bound. Earlier five-car recordings with nine-value arrays still play; they keep their original grid and prompts. `applyReplay()` uses binary search for the surrounding frames and interpolates position and heading. Generator version 2 identifies the seeded circuit implementation; retain old generators before changing its geometry.
...
`src/telemetry.js` validates and strips decision logs, builds response-rate bins and derives timing gaps from frame history. `src/incidents.js` owns the optional injected scenario. `src/sound.js` synthesizes the film soundtrack from recorded speed and event timestamps; it also exports the same samples for MP4 rendering. No third-party sound recording is bundled.

The response log retains allowlisted Choice answers and their probability distributions, model version, receive time and request latency. Each answer also carries a small numeric snapshot of the driver observation. No prose rationale is generated. The selected-driver panel reads this log at the replay clock, so it cannot reveal future replies.

`mechanics.js` handles dissipative spins, cooling-load temperature and sustained-overheat or suspension retirements. The optional incident scenario injects a curb strike and cooling leak; subsequent motion and failures follow the simulation state. `camera.js` moves the chase camera above traffic. Rendering never hides a car because of proximity, retirement or finishing.
