...
const replay = (r) => `/?seasonRace=${encodeURIComponent(r.id)}`;
const phaseName = (r) =>
  r.phase === 'training'
...
function renderDriver() {
  ...
  const rows = season.rounds
    .filter((r) => r.phase === 'training' && r.seed === Number(seed))
    .sort((a, b) => a.generation - b.generation);
  const selected = season.selected?.[driverId];
  document.querySelector('#driver-panel').innerHTML = `
    ...
    <div class="chart-heading"><h3>Flying lap</h3><label>Circuit <select id="training-seed">${season.trainingSeeds.map((s) => `<option ${s === Number(seed) ? 'selected' : ''}>${s}</option>`).join('')}</select></label></div>
    ...
    <div class="prompt-history">${season.generations
      .map((g) => {
...
function render() {
  ...
  <nav class="season-tabs" aria-label="Championship views"><button data-tab="championship" aria-pressed="true">Championship <span>${rounds.length}/3</span></button><button data-tab="training" aria-pressed="false">Prompt lab <span>${season.generations.length} versions</span></button></nav>
  ...
  <section id="training-view" hidden><div class="lab-layout"><aside><label for="driver-select">Driver</label><select id="driver-select">${DRIVERS.map((d) => `<option value="${d.id}" ${d.id === driverId ? 'selected' : ''}>${esc(d.name)}</option>`).join('')}</select><h2>Three versions.<br/>Every attempt saved.</h2><p>Jev chooses a strategy adjustment using each driver’s earlier results. Candidates race the same two circuits; the fastest average finish earns a place in the championship.</p><p>Original prompts can win. Unfinished races receive a penalty; slower attempts stay in the history.</p><p class="note">This optimizes prompts. Model weights stay unchanged. All ten cars race together, so differences also include traffic and API latency.</p></aside><section id="driver-panel"></section></div></section>
