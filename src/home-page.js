  ...
  document.querySelector('#circuit-grid').innerHTML = seeds
    .map((seed) => {
      const rounds = races.filter((r) => r.seed === seed),
        race = rounds.at(-1);
      return `<a class="circuit-card" href="${url(race.id)}"><svg viewBox="0 0 320 200" role="img" aria-label="Circuit ${seed}"><path d="${race.path}"/></svg><div><strong>${circuitName(seed)}</strong><span>${rounds.length} races</span><b>▶</b></div></a>`;
    })
    .join('');
} catch {
