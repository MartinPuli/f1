...
const sessionReady = fetch('/api/status', { cache: 'no-store' })
...
$('#setup-form').onsubmit = async (e) => {
  ...
  $('#setup-key').value = '';
  if (!replay && race.time) await saveRace();
  if (live) {
...
// Serialize writes so an older snapshot can't replace a later finish.
...
function frame(now) {
  ...
  if (replay) {
    ...
    if (race.finished && !finishSaved) {
      ...
      finishSaved = true;
      saveRace();
    } else if (race.running && race.time - lastSaveAttempt >= 15) saveRace();
  }
  scene?.render();
  if (now - lastUi > 150) {
    updateUi();
