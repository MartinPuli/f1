...
function updateUi() {
  ...
  $('#live-status').textContent = showcase
    ? !showcase.playing
      ? 'PAUSED'
      : race.time === 0
        ? 'ON THE GRID'
...
function frame(now) {
  ...
  if (replay) {
    if (showcase) {
      ...
      const view = reelFrame(showcase.elapsed, replay.record.duration);
