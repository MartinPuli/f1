...
export function siteNavigation(current) {
  ...
  header.innerHTML = `<div class="site-header-inner"><a class="site-logo" href="/" aria-label="JEVRACE home"><img src="/logo.svg" alt="JEVRACE"></a><nav aria-label="Main navigation">${items.map(([id, href, label]) => `<a href="${href}" ${id === current ? 'aria-current="page"' : ''}>${label}</a>`).join('')}</nav><a class="site-admin" href="/admin.html">Admin ↗</a></div>`;
