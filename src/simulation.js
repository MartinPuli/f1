...
export const DRIVERS = [
  { id: 'max', name: 'Max JEVstappen', short: 'JEVstappen', number: '01', color: '#ff775b', style: 'Aggressive', risk: 1.08 },
  { id: 'lewis', name: 'Lewis JEVmilton', short: 'JEVmilton', number: '44', color: '#b199eb', style: 'Adaptive', risk: 1.00 },
  { id: 'charles', name: 'Charles LeJEVclerc', short: 'LeJEVclerc', number: '16', color: '#efb83e', style: 'Precise', risk: .94 },
  { id: 'lando', name: 'Lando JEVrris', short: 'JEVrris', number: '04', color: '#58baa0', style: 'Opportunistic', risk: 1.04 },
  { id: 'franco', name: 'Franco ColJEVpinto', short: 'ColJEVpinto', number: '43', color: '#64b4e9', style: 'Patient', risk: .88 },
];
...
export function makeTrack(seed = 42) {
  ...
  const rx=58+rand()*24, rz=39+rand()*20, phase=rand()*Math.PI*2;
  const harmonics=Array.from({length:3},(_,i)=>({frequency:i+2,amplitude:(.10+rand()*.09)/(i+1),phase:rand()*Math.PI*2}));
  const rotation=rand()*Math.PI*2;
  const pts=Array.from({length:96},(_,i)=>{const a=i/96*Math.PI*2+phase;const radius=1+harmonics.reduce((sum,h)=>sum+h.amplitude*Math.sin(a*h.frequency+h.phase),0);const x=Math.cos(a)*rx*radius,z=Math.sin(a)*rz*radius;return new Vector3(x*Math.cos(rotation)-z*Math.sin(rotation),0,x*Math.sin(rotation)+z*Math.cos(rotation));});
...
export function demoDecision(observation,car){
 ...
 return {throttle,brake,steer,confidence:null,label:observation.off_track?'Recovering':brake?'Braking':Math.abs(steer)>.25?'Cornering':throttle?'Accelerating':'Holding'};
}
...
export class Race {
 ...
 tick(realDelta,speed=1){
  ...
  while(this.accumulator>=.025){
   ...
   if(this.time-this.lastFrame>=.1)this.captureFrame();
   if(this.cars.every(c=>c.finished)){this.finished=true;this.running=false;this.addEvent('system','Checkered flag. Race finished.');break;}
   if(this.time>this.limit*100){this.finished=true;this.running=false;this.addEvent('system','Time limit reached. Ranked by distance.');break;}
  }
 }
 captureFrame(){this.lastFrame=this.time;this.frames.push({t:+this.time.toFixed(3),cars:this.cars.map(c=>[c.x,c.z,c.heading,c.speed,c.steer,c.progress,c.lap,c.finished?1:0,c.distance].map(v=>+v.toFixed(4)))});}
