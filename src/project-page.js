...
try {
 ...
 <section id="experiment" class="project-section"><div class="section-intro"><p class="kicker">INSIDE ONE DECISION</p><h2>How Jev drives each car.</h2><p>Jev chooses the intent. The simulator handles steering, grip and contact.</p></div><div class="decision-flow"><div class="observation"><span class="step-label">01 / OBSERVE</span><h3>Franco ColJEVpinto</h3><p class="small">Recorded at ${data.sample.t.toFixed(2)}s</p><dl><div><dt>Speed</dt><dd>${Math.round(data.sample.observation.speed * 3.6)} <small>km/h</small></dd></div><div><dt>Battery</dt><dd>${Math.round(data.sample.observation.battery * 100)}<small>%</small></dd></div><div><dt>Car ahead</dt><dd>${data.sample.observation.ahead ?? '—'} <small>m</small></dd></div></dl></div><div class="decision"><span class="step-label">02 / JEV RESPONSE</span><div class="response-title"><h3>${esc(data.sample.model)}</h3><span>${data.sample.ms}ms</span></div>${[
   'line',
   'pace',
   'power',
 ]
 ...
 <section id="prompts" class="project-section"><div class="section-intro"><p class="kicker">THE PROMPT EXPERIMENT</p><h2>How the prompts changed.</h2><p>Choose a driver, then select a version to read its prompt and watch its race.</p></div><div class="prompt-tools"><label for="story-driver">Driver</label><select id="story-driver">${data.drivers.map((d) => `<option value="${d.id}" ${d.id === 'oscar' ? 'selected' : ''}>${esc(d.name)}</option>`).join('')}</select><label for="story-seed">Circuit</label><select id="story-seed"><option>8912</option><option>2046</option></select></div><div id="prompt-story"></div></section>
 <section class="project-section findings"><div class="section-intro"><p class="kicker">CONFIRMED CHANGES</p><h2>The confirmed improvements.</h2><p>Mean race time across four paired comparisons, on the same circuits.</p></div><div class="improvement-grid">${data.drivers
 ...
 <section class="archive-cta"><div><p class="kicker">30 SAVED RACES</p><h2>Choose your next race.</h2><p>Original decisions, results and 3D replays.</p></div><a class="primary" href="/championship.html">Explore the seasons ↗</a></section></main><footer class="project-footer"><img src="/logo.svg" alt="JEVRACE"><span>Recorded races · real Jev decisions</span><a href="/lab.html">Full results ↗</a></footer>`;
  ...
  if (promptPage) {
    ...
    routes.className = 'project-routes';
    routes.innerHTML =
      '<a href="/championship.html"><strong>Watch races <span>↗</span></strong><p>30 recordings, organized by season.</p></a><a href="/prompts.html"><strong>Explore the prompts <span>↗</span></strong><p>Every version, its instructions and its result.</p></a>';
    document.querySelector('.project-facts').after(routes);
  }
