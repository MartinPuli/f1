...
export const DEFAULT_PROMPT =
  'Complete the race as quickly as possible. Stay on the circuit, avoid contact, and learn from your recent observations.';
const strategies = [
...
export function defaultSettings() {
  return {
    ...
    drivers: DRIVER_IDS.map((id, i) => ({ id, model: 'jev-latest', prompt: strategies[i] })),
  };
}
export function validSettings(value) {
  return (
    ...
    value.drivers.every(
      (d) => validModel(d.model) && typeof d.prompt === 'string' && d.prompt.length <= 1000,
    )
  );
}
export function cleanSettings(value) {
  ...
  return {
    ...
    drivers: DRIVER_IDS.map((id) => {
      ...
      return { id, model: d.model, prompt: d.prompt };
    }),
  };
}
