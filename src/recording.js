...
export function applyReplay(race, recording, time) {
  ...
  const a=frames[lo],b=frames[Math.min(lo+1,frames.length-1)],f=b.t>a.t?Math.min(1,(t-a.t)/(b.t-a.t)):0;
  race.time=t;
  race.cars.forEach((car,i)=>{const p=a.cars[i],q=b.cars[i];const mix=j=>p[j]+(q[j]-p[j])*f;Object.assign(car,{x:mix(0),z:mix(1),heading:p[2]+Math.atan2(Math.sin(q[2]-p[2]),Math.cos(q[2]-p[2]))*f,speed:mix(3),steer:mix(4),progress:mix(5),lap:p[6],finished:!!p[7],distance:mix(8),finishTime:recording.drivers.find(d=>d.id===car.id)?.finishTime});});
}
